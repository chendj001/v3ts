<template>
  <div class="vaw-tab-bar-container">
    <div style="display: flex; align-items: center;">
      <n-icon class="arrow-wrapper" :class="{ 'arrow-wrapper__disabled': leftArrowDisabled }" @click="leftArrowClick">
        <SvgIcon name="left" />
      </n-icon>
      <n-scrollbar ref="scrollbar" :x-scrollable="true" :size="0">
        <n-button v-for="item of getVisitedRoutes" :key="item.path"
          :type="currentTab === item.path ? 'primary' : 'default'" class="tab-item" strong secondary
          style="--n-height: 24px; --n-font-weight: 200" :data="item.path" @click.self="itemClick(item)"
      ...
      <n-icon class="arrow-wrapper" :class="{ 'arrow-wrapper__disabled': rightArrowDisabled }"
        style="transform: rotate(180deg)" @click="rightArrowClick">
        <SvgIcon name="left" />
      </n-icon>
    ...
    <ul v-show="showContextMenu" class="contex-menu-wrapper" :style="contextMenuStyle">
      <li>
        <n-button size="small" :underline="false" type="text" @click="refreshRoute">
          <template #icon>
            <n-icon>
      ...
      <li :disabled="showLeftMenu">
        <n-button size="small" :disabled="showLeftMenu" type="text" @click="closeLeft">
          <template #icon>
            <n-icon>
              <SvgIcon name="arrowLeft" />
            </n-icon> </template>关闭左侧
        </n-button>
      </li>
      <li :disabled="showRightMenu">
        <n-button size="small" :disabled="showRightMenu" type="text" @click="closeRight">
          <template #icon>
            <n-icon>
      ...
      <li>
        <n-button size="small" icon="el-icon-close" type="text" @click="closeAll">
          <template #icon>
            <n-icon>
              <SvgIcon name="close" />
            </n-icon>
...
export default defineComponent({
      ...
      contextMenuOptions: [
        {
          ...
          icon() {
            return h(NIcon, null, {
              default: () => h(SvgIcon, { name: "refresh" }),
            });
          },
        },
        {
  ...
  methods: {
    ...
    isLeftLast(tempRoute: string) {
      return (
        this.getVisitedRoutes.findIndex((it) => it.path === tempRoute) === 0
      );
    },
